...
1. 更新 `<dox.version>` プロパティ `/dox/dox.installer/pom.xml` ファイルのCloud ServicesGit コードを 2022.1.78 に変更します。
1. 変更をコミットし、Cloud Servicesパイプラインを実行して、の 1 月のリリースにアップグレードします。 [!DNL AEM Guides] as a Cloud Service。
