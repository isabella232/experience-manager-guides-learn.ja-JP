---
title: レビュー用にトピックを送信
description: トピックを送信してレビューする方法を説明します
...
- **Web エディター**:個々のトピックまたは DITA マップをレビュー用に送信できます。 レビュータスクを作成するワークフローは、Web エディターと Assets UI で共通です。 異なるのは、レビューワークフローを開始する方法のみです。 Web エディターからレビューワークフローを開始する方法について詳しくは、 [レビュータスクを作成](web-editor-features.md#id215OCJ00JXA) 機能を Web エディターに追加します。

- **Assets UI**:1 つ以上のトピックと DITA マップをレビュー用に送信できます。 Assets UI ワークフローからのレビュー用のドキュメントの共有については、このトピックで説明します。
...
1. を入力します。 **タイトル** タスクの「」と「DITA」の選択 **プロジェクト** 」を選択します。

1. 内 **割り当て先** ドロップダウンフィールドで、トピックをレビュー用に送信するレビュー担当者を選択します。
   ...
   >[!NOTE]
   >
   > レビューワークフローは、プロジェクト固有です。 プロジェクトを作成する場合、チームメンバーをプロジェクトに追加し、グループに割り当てます。 ここでプロジェクトを選択すると、そのプロジェクトの一部であるメンバーを選択できます。 プロジェクトの詳細については、 [DITA プロジェクトの作成](authoring-create-dita-project.md#).
...
1. を選択します。 **期限** レビューの期限をマークする時間
   ...
   >[!NOTE]
   >
   > 期限に達すると、レビュータスクが完了したことを知らせる電子メールがイニシエーターに送信されます。 開始者は、レビュータスクの期限を [レビューダッシュボード](review-manage-tasks-review-dashboard.md#).

1. 次からルートマップを選択： **ルートマップパス**. このルートマップは、レビューコンテンツで使用されるすべての主要な参照と用語集の用語を解決するために使用されます。 ルートマップを選択しない場合、DITA トピックに関連するキー参照や用語集の用語は、レビュー用にトピックを送信する前に解決されません。
...
1. コンテンツページで、レビュー用に共有するトピックのバージョンを選択します。
   ...
   - を選択します。 **バージョンオン** 」オプションを選択し、指定した日時にバージョンを選択する日付と時間を指定します。 指定した日付に使用可能なトピックのバージョンがない場合は、指定した日時の直後に使用可能なバージョンが選択されます。
...
1. クリック **作成** レビュータスクを作成します。
   ...
   レビュータスクが正常に作成されると、確認メッセージが表示されます。 この [ドキュメントの状態](web-editor-document-states.md#) を設定します。
...
DITA マップは、本内のトピックの論理的な構成です。 個々のトピックをレビュー用に送信した場合、レビュー担当者は、ブック内のそのトピックの場所に関する情報を取得しません。 レビュー担当者がレビュー対象のトピックの正確な場所に関する情報を持っている場合、レビュー担当者はレビュー対象のトピックに関するより適切なコンテキストを取得します。
...
で同じ DITA マップを共有して、複数のレビュータスクでのレビューを実行できます。 例えば、DITA マップにトピック A、B、C、D、E がある場合、1 つのレビュータスクで A、B、C を共有してレビューし、別のレビュータスクでトピック C、D、E をレビュー用に送信できます。 レビュープロセスを使用すると、同じトピックとマップファイルを複数のレビュータスクで共有できます。 複数のレビュータスクで共通のトピックを使用する場合、1 つのレビュータスクで指定したコメントが、他のレビュータスクのコメントで上書きまたは結合されることはありません。
