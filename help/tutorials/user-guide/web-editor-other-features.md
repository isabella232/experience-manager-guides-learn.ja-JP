...
- 要素を展開すると、選択したテキストから要素タグを削除し、親要素と結合できます。 例えば、 `p` 要素内の `note` 要素内で、 `p` 要素を使用して、 `note` 要素を選択します。 「要素を展開」オプションは、トピックのパンくずリストのコンテキストメニューに表示されます。 要素の折り返しを解除するには、トピックのパンくずリストから要素を選択し、要素を右クリックしてコンテキストメニューを開き、最後に「要素の折り返しを解除」を選択して要素を削除し、要素のテキストと親要素を結合します。
