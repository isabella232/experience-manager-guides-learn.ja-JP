...
1. クリック **出力**.
   ...
   - **待機中の出力：**
      ...
      リンクをクリックして、 **ダッシュボードを公開** 現在の実行状態を表示します。 すべてのアクティブな公開タスクのリストは、公開ダッシュボードで使用できます。 この **待機中の出力** そして **ダッシュボードを公開**&#x200B;リンクは、生成を待機している出力がある場合、または生成プロセス中の出力がある場合にのみ表示されます。 出力タスクが完了すると、これらのタスクは表示されません。「ダッシュボードを公開」で詳しくは、 [公開ダッシュボードを使用して公開タスクを管理](generate-output-publish-dashboard.md#).
...
1. 「生成された出力」リストで、削除するタスクにポインターを置きます。
