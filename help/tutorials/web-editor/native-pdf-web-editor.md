...
PDFの出力プリセットから、PDFを選択し、条件を適用し、制限を設定して、ユーザーがテンプレートとやり取りする方法を制御し、圧縮や準拠などの詳細設定を行うことができます。
...
1 つの変数または変数の組み合わせを使用して、メタデータを定義できます。 例：`${dc:title} ${dc:docstate}`

変数と文字列の組み合わせを使用することもできます。  例：`View ${dc:title} in ${dc:language}`
