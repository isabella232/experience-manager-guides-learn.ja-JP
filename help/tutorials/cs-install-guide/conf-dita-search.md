...
1. 選択 **一般** ツールのリストから、 **検索Forms** タイル。

1. 内 **検索Forms** リストで、 **アセット管理者の検索レール**.

1. クリック **編集**.
1. 内 **述語を選択** 」タブで、リストの最後までスクロールします。
...
1. クリック **編集**.
1. 内 **述語を選択** タブ、選択 **プロパティの述語** 検索フォーム内の必要な場所にドラッグ&amp;ドロップします。

1. 内 **設定** 「 」タブで、新しく追加された **プロパティの述語** コンポーネント：
...
デフォルトの検索設定では、DITA 内のすべての要素と属性を検索できます `prolog` 要素。 他の要素や属性に基づいて検索する場合は、検索シリアル化ファイルを設定する必要があります。
...
属性：検索する属性のリスト。 属性ごとに、 `attribute` 要素。
...
に示す手順を使用します。 [設定の上書き](download-install-additional-config-override.md#) 設定ファイルを作成します。 設定ファイルで、次の\（プロパティ\）詳細を指定して、メタデータ抽出オプションを設定します。
...
>[!NOTE]
>
> この手順を使用して、除外リストに他のフォルダの場所を追加できます。 インデックスの操作の詳細については、「 [コンテンツの検索とインデックス作成](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/operations/indexing.html?lang=ja).
