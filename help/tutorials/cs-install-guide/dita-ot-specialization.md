...
1. カスタム DITA-OT プラグインを使用するには、次のプロパティを設定します。
   ...
   >[!NOTE]
   >
   > AEM Guides インストーラーは、カスタム DITA-OT プラグインファイルのパスを指定するために使用できる 2 つの環境変数を作成します。 次の環境変数があります。 `DITAOT\_DIR`：ファイルシステム上の DITA-OT ディレクトリのパスを含む。および `DITAMAP\_DIR`：ファイルシステム上で DITA マップコンテンツが抽出されるパスを含みます。
