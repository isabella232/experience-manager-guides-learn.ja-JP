...
定義済みのフィールドに加えて、メタデータ情報を変数やページレイアウトのフィールドとして追加することもできます。 このメタデータはソース DITA マップコンテンツに保存され、ページレイアウトに簡単に挿入できます。 <!--For more information, see [Add fields and metadata](design-page-layout.md#add-fields-and-metadata).-->
...
1. 複数列のページレイアウトをカスタマイズするには、複数列のプロパティを使用します。
   ...
   * **列幅** :ページレイアウト上の要素を列にまたがって配置する場合は、このプロパティを使用する必要があります。 これは、スタイルシートを使用して目的の要素のスタイルを変更することで実現されます。 <!--for more information see _Section explaining style customization_-->.
...
* **複数の列** :ページまたはチャプター目次などの特定の要素の複数列プロパティを設定するプロパティが含まれます。 プロパティとその使用方法について詳しくは、 [複数列のページレイアウトの使用](#multi-column-layout).
